		...
		if ((this.options.groupBy) && (this.options.groupByField !== '')) {
			...
			if ((this.options.sortName != this.options.groupByField)) {
				if (this.data.length > 0) {
					var cItem = this.data[0];
					if (typeof (cItem[that.options.groupByField]) == "string") {
						this.data.sort(function (a, b) {
							return a[that.options.groupByField].localeCompare(b[that.options.groupByField]);
						});
					} else {
						this.data.sort(function (a, b) {
							var sField = that.options.groupByField;
							return a[sField] - b[sField];
						});
					}
				}
			}
		...
		if ((this.options.groupBy) && (this.options.groupByField !== '')) {
			...
			tableGroups.forEach(function (item) {
				...
				html.push('<td',
                    ...
                    '>', (that.options.groupFormatter) ? that.options.groupFormatter(item) : item.name, '</td>'
                );
